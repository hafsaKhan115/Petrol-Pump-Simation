...
def draw_3d_fuel_bar(percentage, vtype):
    ...
    # 1. Dynamic Height based on vehicle type
    ...
    # 2. Smooth Color Transition (Red -> Yellow -> Green)
    # Using 'percentage' to slide between colors
    r = 1.0 - percentage
    g = percentage
    b = 0.0
    
    glPushMatrix()
    # Position it above the vehicle
    glTranslatef(-width/2, 0, h) 
    
    # --- DRAW BACKGROUND (Border) ---
    glColor3f(0.1, 0.1, 0.1) # Darker gray for better contrast
    # The background box
    draw_box(0, -5, 0, width, 5, thickness)
    
    # --- DRAW FILLING BAR (The progress) ---
    # To stop flickering: 
    # We move the bar slightly forward on the Y axis (-5.1 instead of -5)
    # and make it slightly taller/wider (0.1 offset) so it covers the background
    glColor3f(r, g, b)
    if percentage > 0:
...
def show_screen():
    ...
    for v in vehicles:
        draw_vehicle(v)
    # ... inside show_screen function, after draw_vehicle loop ...

    if vehicles:
        front_v = vehicles[0]
        # Show fuel bar if the vehicle is currently at the pump or fueling
        if front_v["state"] in ["fuelling", "moving", "leaving"]:
            # If it hasn't started fueling yet, percentage is 0
            pct = front_v.get("fuel_pct", 0.0)
            
            # Position the bar in the top right or near the queue info
